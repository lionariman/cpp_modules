...
void scavAttacksEnemy(ScavTrap *scav, FragTrap *enemy) {
	int index(0);

	index = std::rand() % 2;
	if (index == 1)
	{
		scav->challengeNewscomer();
...
int main(void) {
	...
	ScavTrap *scavTrap1 = new ScavTrap("Stranger");
	...
	while (true) {
		...
		int rr = std::rand() % 3;
		if (rr == 1)
			scavAttacksEnemy(scavTrap1, clapTrap2);
		else if (rr == 2)
			scavAttacksEnemy(scavTrap1, clapTrap1);
		else if (rr == 3)
			scavAttacksEnemy(scavTrap1, clapTrap3);
	}
